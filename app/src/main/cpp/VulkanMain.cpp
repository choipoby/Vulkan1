...
void CreateVulkanDevice(ANativeWindow* platformWindow,
    ...
    for (auto extName : instance_extensions) {
        if(layerUtil.isExtensionSupported(extName, VK_NULL_HANDLE, nullptr)) {
            LOGI("extName: %s supported", extName);
        } else {
            LOGI("extName: %s not supported", extName);
        }
    }
    for (auto extName : device_extensions) {
        if(layerUtil.isExtensionSupported(extName, VK_NULL_HANDLE, nullptr)) {
            LOGI("extName: %s supported", extName);
        } else {
            LOGI("extName: %s not supported", extName);
...
bool InitVulkan(android_app* app) {
    ...
    for (int bufferIndex = 0; bufferIndex < swapchain.swapchainLength_;
        ...
        VkRenderPassBeginInfo renderPassBeginInfo{
                ...
                .renderArea = {  .offset = {.x = 100, .y = 500,}, .extent = { .width = 500, .height = 400} },
                .clearValueCount = 1,
